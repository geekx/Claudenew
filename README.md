...
- [x] **P2a 主导格式基准**：多 run 段落的译文按「携带字符最多」的 run 套格式，
      段首小段加粗标签不再把整段译文带粗（真实 docx 测试覆盖）
- [x] **性能：单遍分析**：语言检测/术语保护从「分析+回填各一遍」改为单遍复用，
      大文档下该阶段开销减半（调用计数测试锁定）
- [ ] **P2b 完整内联格式**：段内多个格式片段的逐段还原（粗体/斜体混排）、超链接关系
