...
class TranslationMemory:
        ...
        with self._lock:
            with self._get_conn() as conn:
                row = conn.execute(
                    ...
                    (text.strip(), source_lang.lower(), target_lang.lower())
                ).fetchone()
                if row:
                    conn.execute(
    ...
    def store(self, source: str, target: str, source_lang: str, target_lang: str):
        ...
        with self._lock:
            with self._get_conn() as conn:
                conn.execute(
                    '''INSERT INTO tm (source_text, target_text, source_lang, target_lang)
                       VALUES (?, ?, ?, ?)
                       ON CONFLICT(source_text, source_lang, target_lang)
                       DO UPDATE SET target_text=excluded.target_text, use_count=use_count+1, updated_at=datetime("now")''',
                    (source.strip(), target.strip(), source_lang.lower(), target_lang.lower())
                )
            ...
            with self._get_conn() as conn:
                conn.executemany(
                    '''INSERT INTO tm (source_text, target_text, source_lang, target_lang)
                       ...
                       ON CONFLICT(source_text, source_lang, target_lang)
                       DO UPDATE SET target_text=excluded.target_text, use_count=use_count+1, updated_at=datetime("now")''',
                    [(s.strip(), t.strip(), source_lang.lower(), target_lang.lower()) for s, t in pairs if s and t]
                )
...
class DeepSeekTranslator:
    ...
    def create_batch_payload(self, texts: List[str], source_lang: str, target_lang: str, formality: str = "auto") -> dict:
        """创建批量翻译请求体 - 增强版，支持正式度控制"""
    ...
    def translate_batch(self, texts: List[str], source_lang: str, target_lang: str, formality: str = "auto") -> List[str]:
        if not texts:
            return []
        payload = self.create_batch_payload(texts, source_lang, target_lang, formality)
        sep = DEFAULT_CONFIG['TERM_SEPARATOR']
        ...
        for attempt in range(retries):
            try:
                ...
                if len(texts) <= 5:
                    ...
                    results = []
                    for t in texts:
                        individual = self._translate_single(t, source_lang, target_lang, formality)
    ...
    def _translate_single(self, text: str, source_lang: str, target_lang: str, formality: str = "auto") -> str:
        """翻译单个文本段落（用于批量失败时的降级处理）"""
        if not text:
            return text
        sep = DEFAULT_CONFIG['TERM_SEPARATOR']
        try:
            payload = self.create_batch_payload([text], source_lang, target_lang, formality)
            resp = self.session.post(
                self.base_url,
                headers={"Authorization": f"Bearer {self.api_key}", "Content-Type": "application/json"},
                json=payload,
...
def process_ppt_elements(elements: List[PPTElementInfo], term_map, term_re, translator,
    ...
    for elem_info in elements:
        ...
        if needs_translation:
            ...
            # Check TM first
            if tm:
                tm_result = tm.lookup(original_text, source_lang, target_lang)
                if tm_result:
    ...
    if unique_texts:
        ...
        with ThreadPoolExecutor(max_workers=6) as executor:
            batches = [unique_texts[i:i + DEFAULT_CONFIG['BATCH_SIZE']] for i in range(0, len(unique_texts), DEFAULT_CONFIG['BATCH_SIZE'])]
            future_to_batch = {executor.submit(translator.translate_batch, batch, source_lang, target_lang, formality): batch for batch in batches}
            for i, future in enumerate(as_completed(future_to_batch)):
                progress_callback.progress(f"Processing batch {i+1}/{len(batches)}...")
                try:
                    original_batch, translated_batch = future_to_batch[future], future.result()
                    if need_capitalize:
                        translated_batch = [capitalize_first_letter(text) for text in translated_batch]
                    api_translation_map.update(zip(original_batch, translated_batch))
                    # Store new translations in TM
                    if tm:
                        tm.store_batch(list(zip(original_batch, translated_batch)), source_lang, target_lang)
                except Exception as e:
...
def process_text_elements(elements, term_map, term_re, translator, source_lang, target_lang,
    ...
    """处理非PPT元素的文本（Word, Excel）- 支持双语模式、翻译记忆"""
    if not elements:
        progress_callback.error("No text elements found | 未找到文本元素")
        return
    ...
    if unique_texts_for_api:
        ...
        with ThreadPoolExecutor(max_workers=6) as executor:
            batches = [unique_texts_for_api[i:i + DEFAULT_CONFIG['BATCH_SIZE']] for i in range(0, len(unique_texts_for_api), DEFAULT_CONFIG['BATCH_SIZE'])]
            future_to_batch = {executor.submit(translator.translate_batch, batch, source_lang, target_lang, formality): batch for batch in batches}
            for i, future in enumerate(as_completed(future_to_batch)):
                progress_callback.progress(f"Processing batch {i+1}/{len(batches)}...")
                try:
    ...
    try:
        ...
        if ext in ('.xlsx', '.xls'):
            ...
            if elements:
                ...
                process_text_elements(elements, term_map, term_re, translator, source_lang, target_lang, progress_callback, translation_mode, formality)
            ...
            if ppt_elements:
                ...
                process_ppt_elements(ppt_elements, term_map, term_re, translator, source_lang, target_lang, progress_callback, translation_mode, formality)
                progress_callback.status("Saving translated document...")
                doc_obj.save(output_path)
            else:
                progress_callback.status("No text found to translate")
...
class Api:
    ...
    def open_batch_file_dialog(self):
        with self.file_dialog_lock:
            if WINDOW:
                try: 
                    return WINDOW.create_file_dialog(webview.OPEN_DIALOG, allow_multiple=True) or []
                except Exception as e: 
